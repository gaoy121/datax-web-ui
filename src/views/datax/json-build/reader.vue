<template>
  <div class="app-container">
    <RDBMSReader v-show="dataSource!=='hive' && dataSource!=='hbase' && dataSource!=='mongodb' && dataSource!=='starrocks'" ref="rdbmsreader" @selectDataSource="showDataSource" />
    ...
    <StarRocksReader v-show="dataSource==='starrocks'" ref="starrocksreader" @selectDataSource="showDataSource" />
  </div>
</template>
...
import StarRocksReader from './reader/StarRocksReader'
export default {
  name: 'Reader',
  components: { RDBMSReader, HiveReader, HBaseReader, MongoDBReader, StarRocksReader },
  data() {
    return {
      dataSource: ''
    }
  ...
  methods: {
    getData() {
      if (this.dataSource === 'hive') {
        ...
        return this.$refs.starrocksreader.getData()
      } else {
